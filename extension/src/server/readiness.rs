...
pub(crate) struct ReadinessPoller {
    /// `None` = fallback mode (poll everything).
    epfd: Option<i32>,
    ...
    /// Grown to the interest-set size.
    #[cfg(target_os = "linux")]
...
impl ReadinessPoller {
    ...
    /// Departed fds are already closed (dropping the transport closes the
    /// socket), which removes them from the interest list automatically.
