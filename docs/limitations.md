...
| Cross-worker delivery latency (`socket_workers > 1`) | **+1–2 ticks** | All publishes (QoS 0 included) route through the shared outbox so every worker's subscribers see them; QoS 0 loses its no-database fast path. See [enterprise.md → Scaling out](enterprise.md#scaling-out-multiple-socket-workers) for the other multi-worker caveats (per-worker `$share` balancing, crashed-worker session staleness). |
