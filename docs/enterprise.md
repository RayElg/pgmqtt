...
- **All publishes route through the shared outbox** — client publishes (QoS 0 included) and CDC messages alike — and each worker delivers rows past its own cursor (`pgmqtt_outbox_cursors`) to its own subscribers. Slot 0 garbage-collects rows once every cursor has passed them, reclaiming orphaned messages at the same time. This is the explicit trade: QoS 0 gives up its no-database fast path when `socket_workers > 1`. Each worker heartbeats its cursor row (`last_seen`, at least every 30 s); a cursor with no heartbeat for `pgmqtt.outbox_cursor_stale_secs` (default 300, 0 = never) is ignored by GC so a wedged or crash-looping worker cannot pin the watermark and grow the outbox, messages table, and WAL without bound. The ignored worker, if it revives, resumes above the watermark and may have permanently missed messages for its local subscribers — watch the `pgmqtt_outbox_stale_cursors` gauge; it should be 0 in a healthy cluster.
...
- **Slot 0 owns the singleton duties**: metrics flush (counters are in shared memory, so the totals cover all workers), the database-side session-expiry sweep, outbox GC. Every worker additionally sweeps its own in-memory session state. The connection cap from the license is enforced against the cluster-wide connection gauge. That enforcement is conservative across workers: a client reconnecting onto a *different* worker counts as a new connection while its old connection still holds a slot, so at exactly the cap the reconnect is rejected until the old connection drops (keepalive timeout at worst); reconnecting to the same worker is exempt. The HTTP healthcheck is answered by whichever worker the kernel picks — a 200 means "a worker's loop is ticking".
...
- **Shared subscriptions (`$share`)** deliver each message to exactly one member cluster-wide (workers claim each `(message, group)` pair through `pgmqtt_share_claims`), but the winning member is picked by each worker's local rotation — balancing is not globally round-robin. The pick is also not locality-aware: when no group member is connected to the claiming worker, its rotation falls back to any member, including an offline durable session — so a message can be queued to an offline member (delivered when it reconnects) while a member connected to a different worker is passed over.
...
- **Client IDs are capped at 128 bytes when `socket_workers > 1`.** Longer IDs are rejected at CONNECT with reason 0x85 (Client Identifier not valid): every admitted client must be addressable by the fixed-size cross-worker command channel (takeover kicks, admin disconnects, ACL reloads). Single-worker deployments (community and enterprise alike) are unaffected. Empty client IDs are assigned a broker-unique identifier, returned to MQTT 5 clients in the CONNACK Assigned Client Identifier property.
...
| | `outbox_stale_cursors` | gauge | Outbox delivery cursors currently ignored by GC for missing their heartbeat (`socket_workers > 1`; should be 0 — a nonzero value means a wedged socket worker) |
