...
| `pgmqtt.outbox_cursor_stale_secs` | int | `300` | 0 – 86400 | Seconds without a heartbeat before outbox GC ignores a worker's delivery cursor (enterprise `multiprocess`, `socket_workers > 1`). Protects the outbox, messages table, and WAL from unbounded growth when a worker is wedged or crash-looping; the ignored worker, if it revives, resumes above the GC watermark and may have missed messages for its local subscribers. `0` = never ignore a cursor (unbounded growth if a worker wedges). |
